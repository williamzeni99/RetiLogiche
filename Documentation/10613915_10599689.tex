...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
  ...
  \node[state] (getrc) [below=of abilitread, scale=0.6]  {$GET\_RC$};
  \node[state] (getdim) [below=of getrc, scale=0.6]  {$GET\_DIM$};
  \node[state] (readpixel) [below=of getdim, scale=0.5]  {$READ\_PIXEL$};
  \node[state] (getdelta) [below =of readpixel, scale=0.5] {$GET\_DELTA$};
  ...
  \node[state] (waitingpic) [below=of done, scale=0.5]  {$WAITINGPIC$};
  \node[state] (waitmem) [right=of calcshift, xshift=4cm, scale=1]  {$WAIT\_MEM$};

  \draw (start) edge node{} (init)
        (init) edge node{} (abilitread)
        (abilitread) edge node{} (getrc)
        (getrc) edge node{} (waitmem)
        (waitmem) edge node{} (getrc)
        (getdim) edge node{} (waitmem)
        (waitmem) edge node{} (getdim)
        (readpixel) edge node{} (waitmem)
        (readpixel) edge node{} (getdelta)
        (waitmem) edge node{} (readpixel)
        (getdelta) edge node{} (waitmem)
        (waitmem) edge node{} (calcshift)
        (calcshift) edge node{} (waitmem)
        (calcshift) edge node{} (getpixel)
        (getpixel) edge node{} (waitmem)
        (waitmem) edge node{} (calcnewpixel)
        (calcnewpixel) edge node{} (abilitwrite)
        (abilitwrite) edge node{} (writepixel)
        (writepixel) edge node{} (waitmem)
        (writepixel) edge[bend left] node{} (abilitread)
        (waitmem) edge node{} (done)
        (readpixel) edge node{} (waitmem)
