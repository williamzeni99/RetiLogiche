...
La struttura del progetto è stata realizzata mediante due process, chiamati rispettivamente \texttt{UPDATE} e \texttt{STATES}.
Il primo ha il compito di comunicare con la memoria, mentre il secondo contiene gli stati della macchina impiegati nella equalizzazione delle immagini. In questo modo una porzione di codice è adibita esclusivamente alla computazione dei pixel (process \texttt{STATES}), mentre la rimanente si occupa dei segnali di output della \textit{entity} (process \texttt{UPDATE}).\\
...
Il suo funzionamento è reso possibile da tre segnali a lui riservati: \texttt{next\_state}, che identifica la chiamata allo stato prossimo,  \texttt{curr\_state}, che rileva lo stato corrente, e \texttt{prev\_state}, che conserva lo stato precedente.

\subsection{Elenco Stati - Process STATES}

\subsubsection{START}
 \texttt{START} è il primo stato del process \texttt{STATES} ed è stato pensato come fase di attesa iniziale. Questo stato viene invocato in due situazioni differenti: se il segnale di \texttt{i\_rst} viene portato alto, oppure quando il segnale \texttt{i\_start} viene riportato basso dopo la computazione di un immagine. Lo stato \texttt{START} non cambia fino a quando il segnale \texttt{i\_start} non viene portato alto. In quel momento lo stato successivo viene impostato a \texttt{INIT}.
...
\texttt{WAIT\_MEM} è uno stato centrale durante la gestione del flusso di dati. La sua funzione è quella di far ``sprecare" un ciclo di clock al processore. Ciò ci assicura sia in caso di scrittura, sia in caso di lettura, che i segnali in ingresso e in uscita siano letti o scritti correttamente. Nello specifico, questo stato evita che lo stato successivo lavori con dati obsoleti relativi allo stato precedente. In base allo stato chiamante instrada lo stato prossimo in modo opportuno.\\ \\
...
Lo stato \texttt{GET\_DIM} ha lo scopo di aggiornare il segnale \texttt{dim\_address} con l'indirizzo del primo \textit{byte} libero per la scrittura. Il calcolo che ne determina il valore è: $n\_col \cdot n\_row + 2$.\\
...
\textbf{N.B:}\textit{
La moltiplicazione usa segnali \textit{std\_logic\_vector} da $8$ bit e produce un risultato su $16$ bit. Essendo \texttt{dim\_address} un \textit{std\_logic\_vector} su $16$ bit, non incorrono problemi durante il calcolo.
}
...
\texttt{GET\_MINMAX} aggiorna i segnali \texttt{max\_pixel\_value} e \texttt{min\_pixel\_value}, inizializzati rispettivamente con $0$ e $255$, con il valore di \texttt{i\_data}. Questo avviene se e solo se si verifica che \texttt{i\_data} risulti \textit{maggiore} di \texttt{max\_pixel\_value} (in questo caso aggiorna \texttt{max\_pixel\_value}) oppure se \texttt{i\_data} risulti \textit{minore} di \texttt{min\_pixel\_value} (in questo caso aggiorna \texttt{min\_pixel\_value}). Infine, imposta lo stato prossimo a \texttt{READ\_PIXEL}
%non so perche lo mette in viola

\subsubsection{GET\_DELTA}
\texttt{GET\_DELTA} calcola la \textit{differenza} tra \texttt{max\_pixel\_value} e \texttt{min\_pixel\_value} e imposta lo stato prossimo a \texttt{WAIT\_MEM}.

\pagebreak

\subsubsection{CALC\_SHIFT}
Lo stato \texttt{CALC\_SHIFT} si occupa di effettuare la riga $2$ dello speudo-codice riportato al paragrafo $1.2$.
...
La variabile viene quindi shiftata e su di essa viene fatto il seguente controllo: se risulta essere minore di $255$ aggiorniamo il segnale \texttt{new\_pixel} con le otto cifre meno significative di \texttt{pixel\_to\_shift}, altrimenti il segnale \texttt{new\_pixel} viene impostato a $255$. Si noti che \texttt{new\_pixel} è un segnale \textit{std\_logic\_vector} su $8$ \textit{bit}.
...
Successivamente, il segnale \texttt{o\_done} viene portato ad `$1$' e lo stato prossimo impostato a \texttt{ WAITING\_PIC}.


\subsubsection{WAITING\_PIC}
...
\begin{wrapfigure}{r}{0.40\textwidth}
  \begin{tabular}{|l|r|}
    ...
    \hline \hline
    Slice LUTs & 370\\
    \hspace{0.5cm} LUT as Logic & 370\\
    \hspace{0.5cm} LUT as Memory & 0\\
    \hline
    Slice Registers & 268\\
    \hspace{0.5cm} Register as Flip Flop & 237\\
    \hspace{0.5cm} Register as Latch & 31\\
    \hline
    Muxes & 0\\
    \hline
  \end{tabular}
\end{wrapfigure}
A seguito della sintesi attraverso il tool Vivado è emerso il seguente report. Il report mostra il numero di \textit{Look Up Table} utilizzate dal processore e i relativi registri, sia di tipo \textit{Flip FLop}, sia di tipo \textit{Latch}. Come si può vedere il processore non usufruisce di \textit{Mux - Multiplexer}.
